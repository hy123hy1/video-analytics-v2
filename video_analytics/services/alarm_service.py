...
class AlarmConfig:
    ...
    endpoints_intrusion: List[str] = None  # 闯入检测端点
    endpoints_helmet: List[str] = None     # 安全帽检测端点
    endpoints_overcrowd: List[str] = None  # 超员检测端点
    endpoints_smokefire: List[str] = None  # 烟火检测端点
    ...
    def __post_init__(self):
        if self.endpoints is None:
            self.endpoints = []
        if self.endpoints_intrusion is None:
            self.endpoints_intrusion = []
        if self.endpoints_helmet is None:
            self.endpoints_helmet = []
        if self.endpoints_overcrowd is None:
            self.endpoints_overcrowd = []
        if self.endpoints_smokefire is None:
            self.endpoints_smokefire = []
...
class HttpAlarmService(BaseAlarmService):
    ...
    def _get_endpoints_for_event(self, event: DetectionEvent) -> List[str]:
        """根据事件类型获取对应的端点"""
        event_type = event.event_type.value if hasattr(event.event_type, 'value') else str(event.event_type)

        # 根据事件类型选择端点
        if event_type == "intrusion" and self.config.endpoints_intrusion:
            return self.config.endpoints_intrusion
        elif event_type == "no_helmet" and self.config.endpoints_helmet:
            return self.config.endpoints_helmet
        elif event_type == "overcrowd" and self.config.endpoints_overcrowd:
            return self.config.endpoints_overcrowd
        elif event_type == "fire" and self.config.endpoints_smokefire:
            return self.config.endpoints_smokefire

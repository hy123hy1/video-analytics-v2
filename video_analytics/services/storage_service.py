...
class BaseStorageService(ABC):
    ...
    def upload_video(
        ...
        Args:
            ...
            camera_id: 摄像头ID
            timestamp: 时间戳（可选）
            timestamp_str: 时间戳字符串（可选，用于与图片保持一致）
            metadata: 元数据
    ...
    def _generate_object_name(
        ...
        extension: str,
        timestamp: Optional[datetime] = None,
        custom_timestamp_str: Optional[str] = None
    ) -> str:
        """生成对象名称
        ...
        Args:
            ...
            extension: 文件扩展名
            timestamp: 时间戳对象（可选）
            custom_timestamp_str: 自定义时间戳字符串（可选，优先级高于timestamp）
        """
        if custom_timestamp_str:
            timestamp_str = custom_timestamp_str
        else:
